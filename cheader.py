#! /usr/bin/env python
# Compiler Hacker's Reflective Playground - header.py
# Copyright 2008 Dietrich Epp <depp@zdome.net>
...
class Handler(object):
	...
	def handle(self, obj, path):
		body, do_apply = self.apply_header(obj, path, list(file(path, 'r')))
		if do_apply:
			#tpath = path + '#header#'
			f = file(path, 'w')
			for line in body:
...
class HeaderFixer(object):
	...
	def run(self):
		...
		parser = OptionParser()
		parser.add_option("-d", "--update-dates", dest="update_dates",
			help="update copyright dates", action="store_true", default=False)
...
exclude = set(['lib/syntax/read.c'])

class MyHeaderFixer(HeaderFixer):
	def filter_path(self, path):
		return path not in exclude
	def filter_dir(self, path):
		if not super(MyHeaderFixer, self).filter_dir(path):
			return False
		return os.path.normpath(path) != 'avl'
	...
	def header_suffix(self, path):
		return license
	def header_guardname(self, path):
		gname = super(MyHeaderFixer, self).header_guardname
		if path.startswith('include/'):
			return gname('CHRP_' + path[len('include/'):])
		else:
			return gname(path)
